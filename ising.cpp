...
int main(void)
{   
	cout<<"process started for L="<<L<<endl;
    //start time
    ...
    /*
		...
		Orijinal foru kullanmamam�z�n sebebi openmp'de paralel for i�in sadece integer iterasyon de�i�kenine iizn verilmesi
	*/
    tstar=tmax;//tstar� 5.0a setle
    ...
    double forOneEnd=omp_get_wtime();
    ...
    tstar=tcrit_up - deltat_crit;//tstar� 2.39 a setle
    #pragma omp parallel for schedule(dynamic) num_threads(4) 
    for (int i=0; i<20 ;i++ )
	{
		do_step(spins, neighs, tstar, N, h, energy, gen, ran_u, ran_pos, m);
    ...
    double forTwoEnd=omp_get_wtime();    
	...
	//orijinal for(tstar = tcrit_down - deltat; tstar >= tmin; tstar -= deltat) // 20 kere 0.2<=tstar<2.2 ,, --0.1
	 tstar=tcrit_down- deltat;// tstar� 2.1 e setle
	#pragma omp parallel for schedule(dynamic) num_threads(4) 
    for (int i=0; i<19; i++)
	{
	    do_step(spins, neighs, tstar, N, h, energy, gen, ran_u, ran_pos, m);
    ...
    double forThreeEnd=omp_get_wtime();    
    cout<<"3rd for took "<<forThreeEnd-forThreeStart<<"  sec"<<endl;
...
void initialize(bool spins[SIZE], mt19937& gen, uniform_int_distribution<int>& brandom)
{
    ...
    cout<<"initialize took "<<iEnd-iStart<<endl;
	return;
}
...
void get_neighbors(int neighs[SIZE][4])
{
    ...
    double gnEnd=omp_get_wtime();;
    cout<<"get_neighbors took  "<<gnEnd-gnStart<<endl;
	return;
}
...
void do_step(bool spins[SIZE],  int neighs[SIZE][4], double tstar, int N, double h[5], double& energy, mt19937& gen, uniform_real_distribution<double>& ran_u, uniform_int_distribution<int>& ran_pos, double m[DATA])
{
	...
	//#pragma omp parallel for schedule(dynamic) shared(spins, neighs, h,  energy, gen, ran_u, ran_pos)
	for (j=0; j < 1100; j++)
	{
    ...
    //hata!!! #pragma omp parallel for schedule(dynamic) return value 3221225725
	for (i=0; i < N; i++)
	{
...
void do_step_wolff(bool spins[SIZE],  int neighs[SIZE][4], double tstar, int N, double& energy, mt19937& gen, uniform_real_distribution<double>& ran_u, uniform_int_distribution<int>& ran_pos, double m[DATA])
{
   ...
   //hata!!! #pragma omp parallel for schedule(dynamic) return value 3221225725
    ...
    //Finish the average
    for (i=0; i < DATA; i++)  m[i] /= (1.0 * N);
    double dswEnd=omp_get_wtime();
    cout<<"do_step_wolf took"<<dswEnd-dswStart<<endl;
	return;
}

//Flip a spin via Metropolis(Nerdeyse hi� zaman harcam�yor)
void flip_spin(bool spins[SIZE], int neighs[SIZE][4], double h[5], double& energy, mt19937& gen, uniform_real_distribution<double>& ran_u, uniform_int_distribution<int>& ran_pos)
{
...
//Compute the magnetization(Nerdeyse hi� zaman harcam�yor)
double magnetization(bool spins[SIZE])
{
...
//Computes the energy of the system(i�lem hacmi k���k)
double get_energy(bool spins[SIZE], int neighs[SIZE][4])
{   
    double getEnergyStart=omp_get_wtime();
    int i; //Counters
    int sum_neigh;
    ...
    double getEnergyEnd=omp_get_wtime();
    cout<<"get energy took  "<<getEnergyEnd-getEnergyStart<<endl;
...
void add_to_cluster(bool spins[SIZE], int neighs[SIZE][4], int pos, double& energy, double p, mt19937& gen, uniform_real_distribution<double>& ran_u)
{
	...
	//energy,p,gen,ran_u can be shared
    ...
    for (i=0; i < 4; i++)
    {
        ...
        if (spins[n] != spins[pos]) //Check if it the same (remember we flipped)
        {
            ...
            #pragma omp task shared(energy,gen,ran_u)
            if (ran_u(gen) < p)
            {
